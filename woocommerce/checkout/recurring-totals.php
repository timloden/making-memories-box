...
<tr class="recurring-totals d-none">
    <th class="pt-3" colspan="2"><?php esc_html_e( 'Recurring totals', 'woocommerce-subscriptions' ); ?></th>
</tr>
...
<?php continue; ?>
<?php endif; ?>
<tr class="cart-subtotal recurring-total d-none">
    <?php
					if ( $display_th ) :
						$display_th = false;
						?>
    <th rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>">
        <?php esc_html_e( 'Subtotal', 'woocommerce-subscriptions' ); ?></th>
    <td data-title="<?php esc_attr_e( 'Subtotal', 'woocommerce-subscriptions' ); ?>">
        <?php wcs_cart_totals_subtotal_html( $recurring_cart ); ?></td>
    <?php else : ?>
...
<?php
						...
						?>
<tr class="cart-discount coupon-<?php echo esc_attr( $code ); ?> recurring-total d-none">
    <?php
								if ( $display_th ) :
									$display_th = false;
									?>
    <th rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>">
        <?php wc_cart_totals_coupon_label( $coupon ); ?></th>
    <td data-title="<?php wc_cart_totals_coupon_label( $coupon ); ?>">
        <?php wcs_cart_totals_coupon_html( $recurring_coupon, $recurring_cart ); ?>
        <?php
									echo ' ';
...
<tr class="fee recurring-total d-none">
...
<?php
							...
							?>
<tr class="tax-rate tax-rate-<?php echo esc_attr( sanitize_title( $recurring_code ) ); ?> recurring-total d-none">
    <?php
								if ( $display_th ) :
									$display_th = false;
									?>
    <th><?php echo esc_html( $recurring_tax->label ); ?></th>
    <td data-title="<?php echo esc_attr( $recurring_tax->label ); ?>">
        <?php echo wp_kses_post( wcs_cart_price_string( $recurring_tax->formatted_amount, $recurring_cart ) ); ?></td>
    <?php else : ?>
    <th></th>
...
<tr class="order-total recurring-total">
    <?php
				if ( $display_th ) :
					...
					?>
    <th rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>">
        <?php esc_html_e( 'Recurring total', 'woocommerce-subscriptions' ); ?></th>
    <td data-title="<?php esc_attr_e( 'Recurring total', 'woocommerce-subscriptions' ); ?>">
        <?php wcs_cart_totals_order_total_html( $recurring_cart ); ?></td>
    <?php else : ?>
